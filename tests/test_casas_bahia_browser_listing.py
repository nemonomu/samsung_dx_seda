...
class MergeTests(unittest.TestCase):
    ...
    def merge(self, raw=None, payloads=None, url=URL):
        with patch("requests.sessions.Session.request", side_effect=AssertionError("network forbidden")):
            return browser._enrich_document(raw or document(), url, payloads or [{"Ofertas": [offer()]}])
    ...
    def test_duplicate_identical_offer_allowed_conflict_rejected(self):
        self.merge(payloads=[{"Ofertas": [offer(), offer()]}])
        with self.assertRaisesRegex(browser.EvidenceError, "conflicting_price_offers"):
            self.merge(payloads=[{"Ofertas": [offer(), offer(price=1100)]}])

    def test_wrong_product_sku_or_seller_rejected(self):
        for wrong in (offer(product_id=999), offer(sku=999), offer(seller=999)):
            with self.subTest(wrong=wrong), self.assertRaisesRegex(browser.EvidenceError, "price_or_seller_identity_incomplete"):
                self.merge(document([product(seller=7)]), [{"Ofertas": [wrong]}])

    def test_ambiguous_seller_rejected(self):
        with self.assertRaisesRegex(browser.EvidenceError, "ambiguous_listing_seller"):
            self.merge(payloads=[{"Ofertas": [offer(seller=7), offer(seller=8)]}])

    def test_missing_price_rejected(self):
        bad = offer()
        bad["PrecoVenda"]["Preco"] = None
        bad["DescontoFormaPagamento"] = {}
        with self.assertRaisesRegex(browser.EvidenceError, "missing_listing_price"):
            self.merge(payloads=[{"Ofertas": [bad]}])
    ...
    def test_page_sort_and_source_identity_rejected(self):
        for raw, url, code in ((document(page=2), URL, "requested_page_mismatch"),
                               ...
                               (document([dict(product(), href="/wrong/p/999")]), URL, "listing_identity_invalid")):
...
class SessionTests(unittest.TestCase):
    ...
    def test_recoverable_failure_then_success_uses_two_navigation_traces(self):
        session = Mock()
        failure = {"success": False, "error": "price_or_seller_identity_incomplete", "trace": [{"status_code": 200, "price_coverage": {"missing_price_identity_count": 1}}]}
    ...
    def test_structural_failures_never_retry(self):
        for error in ("requested_page_mismatch", "requested_sort_mismatch", "listing_identity_invalid", "ambiguous_listing_seller", "conflicting_price_offers"):
