"""Offline mode-4 tests. All network/browser launches are mocked."""
...
from seda.casas_bahia import browser_api_url_first as api
from seda.casas_bahia import browser_listing_url_first as browser_listing, price_api, search_api
...
class ContextTests(unittest.TestCase):
    ...
    def test_missing_page_is_explicit_request_context_only(self):
        ...
        self.assertEqual("100", str(copied["products"][0]["idSku"]))
    ...
    def test_source_alias_conflict_keeps_url_and_quarantines_metadata(self):
        data, _ = self.context({"products": [dict(product(), idSku=101)]})
        row = data["products"][0]
        self.assertEqual("/smart-tv/p/100", row["url"])
        self.assertEqual("100", str(row["idSku"]))
        self.assertFalse(row.get("id"))
        self.assertTrue(row["_casas_listing_price_pending"])

    def test_invalid_optional_alias_does_not_block_url(self):
        for bad in (0, True, {}, [], "bad"):
            with self.subTest(bad=bad):
                data, _ = self.context({"products": [dict(product(), idSku=bad)]})
                self.assertEqual("/smart-tv/p/100", data["products"][0]["url"])
                self.assertTrue(data["products"][0]["_casas_listing_price_pending"])

    def test_url_sku_is_authoritative_over_optional_alias(self):
        data, _ = self.context({"products": [dict(product(), url="/correct/p/999")]})
        self.assertEqual("999", str(data["products"][0]["idSku"]))
        self.assertFalse(data["products"][0].get("id"))
...
class SessionTests(unittest.TestCase):
    ...
    def test_price_failure_keeps_urls_without_repeating_search(self):
    ...
    def test_wrong_price_identity_keeps_product_without_quote(self):
        values = [self.good()[0], response({"Ofertas": [offer(sku=999)]})]
        with patch.object(api, "_browser_fetch", side_effect=values * 3):
            result = self.session.fetch(URL)
        self.assertTrue(result["success"])
        self.assertEqual(1, result["trace"][-1]["price_pending_products"])
        products = extract_next_data(result["text"])["props"]["pageProps"]["initialState"]["search"]["results"]["products"]
        self.assertEqual("/smart-tv/p/100", products[0]["url"])
        self.assertFalse(products[0].get("price"))

    def test_price_exception_is_safe_and_does_not_retry_search(self):
        with patch.object(api, "_browser_fetch", side_effect=[self.good()[0], RuntimeError("private-canary")]) as fetch:
            result = self.session.fetch(URL)
        self.assertTrue(result["success"])
        self.assertEqual(2, fetch.call_count)
        self.assertNotIn("private-canary", json.dumps(result))
        self.assertEqual("browser_api_RuntimeError", result["trace"][-1]["price_error"])

    def test_document_change_during_price_is_still_structural_failure(self):
        changed = ({"status_code": 200, "error": "document_navigation_during_api_call"}, None)
        with patch.object(api, "_browser_fetch", side_effect=[self.good()[0], changed] * 3):
            result = self.session.fetch(URL)
        self.assertFalse(result["success"])
        self.assertEqual("document_navigation_during_api_call", result["error"])

    def test_partial_prices_keep_all_urls_and_order(self):
        products = [product(100, 10), product(200, 20), product(300, 30)]
        with patch.object(api, "_browser_fetch", side_effect=[response({"products": products}),
                response({"Ofertas": [offer(100, 10), offer(300, 30)]})]) as fetch:
            result = self.session.fetch(URL)
        self.assertTrue(result["success"])
        self.assertEqual(2, fetch.call_count)
        self.assertEqual(3, result["products"])
        self.assertEqual(1, result["trace"][-1]["price_pending_products"])
        from seda.parsers import parse_listing

        rows = parse_listing(result["text"], "Casas Bahia", "https://www.casasbahia.com.br", URL)
        self.assertEqual(["100", "200", "300"], [row["product_url"].split("/")[-1] for row in rows])
        self.assertTrue(rows[0]["final_sku_price"])
        self.assertFalse(rows[1]["final_sku_price"])
        self.assertTrue(rows[2]["final_sku_price"])
