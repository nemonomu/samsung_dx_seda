...
class NetworkTests(unittest.TestCase):
    ...
    def test_exact_get_and_completed_response_required(self):
        self.assertTrue(self.evidence()["request_response_verified"])
        self.assertFalse(self.evidence(events()[:-1])["request_response_verified"])

    def test_other_page_sort_or_business_params_rejected(self):
        for bad in (SEARCH.replace("page=1", "page=2"), SEARCH.replace("maisvendidos", "menorpreco"), SEARCH + "&regionid=other"):
            with self.subTest(bad=bad):
                self.assertFalse(self.evidence(events(url=bad))["request_response_verified"])

    def test_wrong_frame_or_loader_rejected(self):
    ...
    def test_cache_or_service_worker_response_rejected(self):
        for field in ("fromDiskCache", "fromServiceWorker"):
            value = events()
            value[1]["params"]["response"][field] = True
            self.assertFalse(self.evidence(value)["request_response_verified"])

    def test_post_body_must_match(self):
        ...
        self.assertFalse(self.evidence(events(method="POST", body={}), method="POST", body=body)["request_response_verified"])

    def test_duplicate_matching_requests_rejected(self):
        value = events()
        more = events()
        for item in more:
            item["params"]["requestId"] = "r2"
        self.assertFalse(self.evidence(value + more)["request_response_verified"])
    ...
    def test_browser_fetch_passes_only_given_browser_headers(self):
        ...
        self.assertEqual(15, driver.set_script_timeout.call_args.args[0])
...
class SessionTests(unittest.TestCase):
    def setUp(self):
        ...
        with patch.object(browser_listing, "_BrowserSession", return_value=self.browser):
