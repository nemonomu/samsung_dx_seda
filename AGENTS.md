...
- Preserve all four Casas listing modes: 1=REST API, 2=hybrid, 3=UC + browser API, 4=UC + API URL-first. Keep modes 1/2/3 collection behavior intact; URL-first admission and missing-price Detail recovery are exclusive to mode 4. After each Casas listing code change, explicitly verify the final selected mode in the batch and report it. The current default is 1; change it only when requested. See `docs/casas_listing_modes.md`.
