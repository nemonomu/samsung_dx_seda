"""Mode 4: URL-first browser APIs with same-Chrome SSR after search HTTP 403.
...
from seda.casas_bahia import browser_listing_url_first as browser_listing
from seda.casas_bahia.browser_probe_session import ProbeBrowserSession


class URLFirstProbeBrowserSession(ProbeBrowserSession):
    """Keep established Chrome startup; only listing admission is URL-first."""
    fetch = browser_listing._BrowserSession.fetch
...
def _parser_context(data, requested):
    ...
    for product in products:
        ...
        identity = browser_listing._product_url_sku(product)
        if not identity:
            raise browser_listing.EvidenceError("listing_identity_invalid")
        if identity in seen:
            raise browser_listing.EvidenceError("duplicate_listing_sku")
        seen.add(identity)
        if product.get("idSku") in (None, ""):
            aliases += 1
    # Normalize optional identifiers before building price requests. A failed
    # quote join must never turn a valid product URL into a failed listing.
    browser_listing._attach_optional_prices(products, [])
    return copied, {
...
def _pending_counts(products):
    return {"price_pending_products": sum(p.get("_casas_listing_price_pending") is True for p in products),
            "seller_pending_products": sum(p.get("_casas_listing_seller_pending") is True for p in products)}


class _APISession:
    def __init__(self):
        self.browser = URLFirstProbeBrowserSession()
        self.request_session_id = str(uuid.uuid4())
        self.last_search_finished = None
        print("[seda] casas_bahia mode=4 uc_api_url_first execution_profile=successful_local_probe "
    ...
    def _ssr_fallback(self, url, timeout, trace, *, recovery_pending=False):
        ...
        navigation_trace = []
        print(f"[seda] casas_bahia mode=4 page={requested[1]} fallback=browser_ssr "
        ...
        return {"success": success, "text": html, "status_code": item["status_code"],
                "method": "uc_api_url_first+browser_ssr", "error": item.get("error", ""),
    ...
    def _bootstrap(self, url, timeout):
        if self.bootstrap_attempted:
            if self.bootstrap_error:
                try:
                    print("[seda] casas_bahia mode=4 uc_api_url_first bootstrap_failure_reused=true "
        ...
        self.bootstrap_attempted = True
        started = time.monotonic()
        item = {"method": "uc_api_url_first", "stage": "bootstrap", "status_code": 0, "document_navigations": 1}
        print("[seda] casas_bahia mode=4 uc_api_url_first bootstrap_navigation=1", flush=True)
        try:
            # The established SSR validator verifies this first real page only.
            # Its products are never returned as mode-4 listing output.
            ...
            print(f"[seda] casas_bahia mode=4 uc_api_url_first stage=bootstrap status={self.bootstrap_status} "
        ...
        return {"method": "uc_api_url_first", "stage": "bootstrap", "status_code": 200,
    ...
    def fetch(self, url, timeout=None):
        ...
        for attempt in range(1, limit + 1):
            ...
            item = {"method": "uc_api_url_first", "page": int(requested[1]), "attempt": attempt,
            ...
            try:
                ...
                prices = None
                price_result = {"status_code": 0, "error": "empty_price_items"}
                if product_items or sku_items:
                    ...
                    try:
                        ...
                        price_result = {"status_code": 0, "error": _safe_error(exc)}
                item["price"] = price_result
                if price_result.get("error"):
                    if price_result["error"] == "document_navigation_during_api_call":
                        raise browser_listing.EvidenceError(price_result["error"])
                    item["price_error"] = price_result["error"]
                    prices = None
                item["stage"] = "validation"
                raw = search_api._as_next_data_html(parser_data, url)
                html, ids, row_count = browser_listing._enrich_document(raw, url, [prices] if prices else [])
                from seda.parsers import extract_next_data

                enriched_products = extract_next_data(html)["props"]["pageProps"]["initialState"]["search"]["results"]["products"]
                item.update(_pending_counts(enriched_products))
                ...
                item.update({"stage": "complete", "status_code": 200, "products": len(ids),
                             ...
                             "elapsed_seconds": round(time.monotonic() - started, 3)})
                print(f"[seda] casas_bahia mode=4 uc_api_url_first page={requested[1]} attempt={attempt}/{limit} "
                      f"products={len(ids)} rows={row_count} "
                      f"price_pending={item['price_pending_products']} "
                ...
                return {"success": True, "text": html, "status_code": 200, "method": "uc_api_url_first",
        ...
        return {"success": False, "text": "", "status_code": trace[-1]["status_code"],
                "method": "uc_api_url_first", "error": trace[-1]["error"], "trace": trace}


def fetch_listing(url, timeout=None):
    """Max three search attempts; optional price failure keeps product URLs."""
    global _SESSION
    with _LOCK:
        try:
            ...
            print(f"[seda] casas_bahia mode=4 uc_api_url_first error={error}", flush=True)
            status = exc.status_code if isinstance(exc, _BootstrapError) else 0
            trace = exc.trace if isinstance(exc, _BootstrapError) else [{"method": "uc_api_url_first", "status_code": 0, "error": error}]
            return {"success": False, "text": "", "status_code": status, "method": "uc_api_url_first",
